...
public class Lexer {
    ...
    protected Token matchALPHA() throws LexerException {
        ...
        if (((ch >= 'A') && (ch <= 'Z')) || ((ch >= 'a') && (ch <= 'z'))) {
            this.consumeChar();
            return new Token(Token.TokenType.ALPHA, "" + (char) ch);
        } else
    ...
    public Token matchHexDigit() throws LexerException {
        ...
        this.consumeChar();
        return new Token(Token.TokenType.HEXDIGIT, buffer.toString());
    }
    ...
    public Token matchDigit() throws LexerException {
        ...
        buffer.append(ch);
        this.consumeChar();
        return new Token(Token.TokenType.DIGIT, buffer.toString());
    }
    ...
    public Token readToken() throws LexerException {
        char ch = charLookAhead(1);
        if (ch == 65535) return new Token(Token.TokenType.EOI, "eoi");
        switch (ch) {
            case ':': {
                this.matchChar(':');
            ...
            case '~': {
                this.matchChar('~');
                return new Token(Token.TokenType.TILDA, "~");
            }
            case '.': {
                this.matchChar('.');
            ...
            case '%': {
                ...
                if (Character.isDigit(charLookAhead(2)) && hexDigitPostfix(charLookAhead(3))) {
                    ...
                    return new Token(Token.TokenType.PERCENT, "%");
            }
            case '_': {
                this.matchChar('_');
                return new Token(Token.TokenType.UNDERSCOPE, "_");
            }
            default: {
