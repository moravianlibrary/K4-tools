...
public class ImageserverUtils {
    ...
    private ChannelSftp getSftpConnection(String user, String host, String password, boolean useConfig) throws JSchException {
        JSch jsch = new JSch();

        Session session;
        if (!useConfig && configuration.getPasswordWorkspace() == null) {
            ...
            if(password != null) {
                ...
                AccessProvider ap = AccessProvider.getInstance();
                jsch.addIdentity(ap.getImageserverPrivateKey(), ap.getImageserverPrivatePassphrase());
                session = jsch.getSession(user, host, 22);
            }
        }
